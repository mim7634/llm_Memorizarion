...
class TRAIN_MODEL:
    def __init__(self, data_dir='data', ppl_stop=True, ppl_target=1.001, num_epochs=1000000, learning_rate=1e-3, snapshot_interval=1000, model_base_dir='model', n_layer=4, n_head=8, n_embd=256, data_size="", patience=3000, min_delta=0.0001):
        ...
        self.n_head = n_head
        self.n_embd = n_embd # ここで受け取った embd がクラス変数になる
        ...
        self.data_size = data_size # ここで受け取った size がクラス変数になる
    ...
    def run_training(self):
        ...
        for epoch in progress_bar:
            ...
            if epoch % self.snapshot_interval == 0:
                ...
                # ここで self.n_embd, self.data_size が正しく渡っているか確認
...
def execute_trial(embd, size):
    ...
    trainer = TRAIN_MODEL(
        ...
        ppl_target=1.001, 
        n_embd=embd,   # ここが 16 になっていませんか？ embd変数を使う必要があります
        data_size=size # ここが 20 になっていませんか？ size変数を使う必要があります
    )
...
if __name__ == '__main__':
    ...
    # ★ 32 から再開
    embd_list = [32, 40, 48, 56, 64, 72, 80, 88, 96, 104, 112, 120, 128, 136]
    ...
    for embd in embd_list:
        ...
        # ==========================================================
        # ★復帰ロジック: 5500失敗(仮定) -> 700と5500の間(2250)から再開
        # ==========================================================
        if embd == 32:
            print(">> [RESUME MODE] 5500失敗(仮定) -> 700と5500の間(2250)から再開します")
            try:
                low_idx = data_size_list.index(700)   # 成功ライン
                high_idx = data_size_list.index(5500) # 失敗ライン
                last_success_size = 700
                ...
                print("エラー: data_size_list 設定不整合")
                exit()
        else:
            low_idx = 0
