...
if __name__ == '__main__':
    # 【実験設定】
    target_embd = 32
    target_data_dir = "data/1000" # データの格納ディレクトリ (TRAIN_MODEL.__init__ で使用される DATA_DIR を上書きする想定)
    target_ppl = 1.001            # 暗記成功と判断する PPL ターゲット
    
    # 既存の TRAIN_MODEL クラスは data_dir='data' に固定されているため、
    # 以下の修正が必要です：
    # 1. TRAIN_MODEL の __init__ に data_dir 引数を追加し、self.DATA_DIR と self.TRAIN_PATH を設定可能にする。
    # 2. あるいは、TRAIN_MODEL.DATA_DIR を直接上書きする。
    
    # ここでは、TRAIN_MODEL の __init__ に data_dir='data/1000' を渡せるようにTRAIN_MODELクラス自体に微修正が入ると仮定し、
    # その引数にデータを渡します。

    print(f"\n{'='*30}")
    print(f"単一訓練開始: n_embd = {target_embd}, Data Path = {target_data_dir}")
    print(f"{'='*30}")

    trainer = TRAIN_MODEL(
        # PPL目標に達するまで学習するため、エポック数は非常に長く設定
        num_epochs=1000000, 
        snapshot_interval=500, # 途中経過の保存頻度
        ppl_stop=True,         # PPL目標到達時に停止
        ppl_target=target_ppl, 
        learning_rate=1e-3,    # 訓練を速めるために学習率を調整
        n_embd=target_embd,
        # model_basa_dir='model' (デフォルト)
        # n_layer=4, n_head=8 (デフォルト)
    ) 
    
    # TRAIN_MODEL クラスの self.DATA_DIR と self.TRAIN_PATH を手動で上書き
    # data/1000/train_data.txt を参照させる
    trainer.DATA_DIR = target_data_dir
    trainer.TRAIN_PATH = os.path.join(trainer.DATA_DIR, "train_data.txt")
    
    # 訓練開始
    trainer.run_training()
    
    print(f"\n--- 訓練完了 ---")
    print(f"最終モデルフォルダ名: {trainer.model_folder_name}")